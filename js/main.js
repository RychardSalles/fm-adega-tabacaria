...
const whatsHello = `${whatsBase}?text=${encodeURIComponent(`Olá! Vim pelo site da ${STORE_NAME}.`)}`;

["whatsLink", "whatsCta", "floatWhats"].forEach((id) => {
  const el = document.getElementById(id);
  if (el) el.href = whatsHello;
});
["instaLink", "instaCta"].forEach((id) => {
  const el = document.getElementById(id);
  if (el) el.href = STORE_CONFIG.instagramUrl;
});
...
function buildItemWhatsAppLink(product) {
  ...
  const msg = `Olá, ${STORE_NAME}! Tenho interesse no item *${product.name}*${preco}.\nAinda tem disponível? Sou maior de 18 anos.`;
...
function buyButtonHtml(product) {
  if (!product.available) {
    return `<span class="product-buy is-disabled">Indisponível</span>`;
  }
