...
O site funciona sozinho sem nenhuma configuração — ele mostra um catálogo padrão. Para conseguir **editar os itens pelo painel** (`admin.html`) e essas mudanças aparecerem pra todo mundo que visita o site, é preciso ligar o site a um banco de dados gratuito chamado **Supabase**. Leva uns 10-15 minutos, e não custa nada no plano gratuito.

> **Importante:** use um projeto Supabase **separado** do da FM Sabor Caseiro.
> Os dois sites usam a tabela `produtos`; se compartilharem o mesmo projeto, um
> catálogo sobrescreve o outro.

## 1. Criar o projeto no Supabase

1. Acesse **app.supabase.com** e entre (pode usar conta do GitHub ou e-mail).
2. Clique em **"New project"**.
3. Dê um nome, por exemplo `fm-adega-tabacaria`. Escolha uma senha para o banco (guarde num lugar seguro — não é usada no site) e a região mais próxima (**South America (São Paulo)**).
...
-- Qualquer pessoa pode LER o catálogo (necessário para o site funcionar)
...
2. Na primeira vez que o painel abrir com a tabela vazia, ele mesmo copia o catálogo padrão para a nuvem automaticamente — não precisa clicar em nada.
...
- Excluir itens que saíram do catálogo
